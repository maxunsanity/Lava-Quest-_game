import { botIdToIndex, elimIndicesForLevel } from './simulation.js'
...
/** @param {DataPack} data @param {ElRefs} els */
export function createGame(data, els) {
  const bots = [...data.botConfigRaw].sort((a, b) => botIdToIndex(a.bot_id) - botIdToIndex(b.bot_id))
  /** @type {Set<number>} */
  let alive = new Set(bots.map((_, i) => i))
  let clearsCompleted = 0
  const clearsForRank = bots.map(() => 0)

  let busy = false
  /** 레벨 N 시도 중 실패 시 N (0클리어 탈락 순위용). 성공·로비 복귀 시 null */
  let meFailedAttemptLevel = /** @type {number | null} */ (null)
  ...
  function startTimerIfNeeded() {
    ...
    if (els.lobbyTimer) els.lobbyTimer.textContent = fmtClock(timerRemainMs)
    if (els.clearTimer) els.clearTimer.textContent = fmtClock(timerRemainMs)
  ...
  return {
    startMatchingAnimated() {
      ...
      if (!stack) {
        console.warn('Matching stack not found, skipping to lobby...')
        void lobbyEnter()
        return
      }
    ...
    attachHandlers() {
      ...
      document.getElementById('btn-success')?.addEventListener('click', async () => {
        ...
        try {
          ...
          const elimIx = elimIndicesForLevel(data.eliminationSchedule, clearsCompleted)
          await clearScene?.playClearSuccessAnimation(elimIx, clearsCompleted)
          for (const ix of elimIx) {
      ...
      document.getElementById('btn-reward-confirm')?.addEventListener('click', () => {
        showToast(null, '세션을 종료합니다. 다시 시작하려면 새로고침하세요.')
      })
