    ...
    fleaIntroPromise = (async () => {
      ...
      const tasks = sortedAlive.map((idx) => {
        ...
        return delay(stagger).then(() =>
          tweenFleaHops(mesh, t, { hops: 5, durMs: 142, seed: idx * 997 + 13 }),
        )
      })
  ...
  async function playClearSuccessAnimation(elimPlan, clearsAfter) {
    ...
    if (alive.has(0)) {
      ...
      if (oldT && newT && mesh) {
        ...
        await tweenWorldPos(mesh, { x: newT.x, y: newT.y, z: newT.z }, 520, quadOut)
      }
    }
    ...
    for (const step of elimPlan) {
      ...
      const waitMs = Math.max(0, d - prevScheduled)
      prevScheduled = d
      await delay(waitMs)
      ...
      await tweenWorldPos(mesh, dest, 780, easeOutCubic)
    ...
    for (const wave of waves) {
      ...
      const tw = wave.map((ix) => {
        ...
        const mesh = sprites[ix]
        if (!t || !mesh) return Promise.resolve()
        return tweenWorldPos(mesh, { x: t.x, y: t.y, z: t.z }, 560, quadOut)
      })
