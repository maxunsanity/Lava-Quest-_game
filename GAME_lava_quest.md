...
implementation_truth:
  ...
  ui_shell:
    ...
    matching_dom: "MATCHING은 .screen-body.screen-body--matching 안에 lq-title·grand-prize·matching-box·#avatar-stack 순. #btn-matching-continue는 .screen-footer 안(클래스 btn-primary btn-footer). 버튼이 .hidden일 때 빈 푸터가 자리를 차지하지 않도록 :has(#btn-matching-continue.hidden)로 푸터 패딩·배경 제거."
...
| `--lq-status-fixed-height` | ~4.35rem | 로비·클리어 상단 메시지 박스(`.clear-message`) 동일 높이로 스크롤·박스 정렬 |
...
- **RANKING**: 하단 고정 CTA 없음(헤더 닫기만) — 위 패턴의 예외.
...
## Agent Audit Log (단서 · 작업 위주)

본 절은 **규범적 YAML mechanics와 별도**로, 실제 편집 세션에서 겪은 **증상 → 원인 → 조치**를 남긴다. 후속 에이전트는 여기만 읽어도 동일 버그를 재도입하지 않도록 한다.

| 증상 | 원인 | 조치 |
|------|------|------|
| 연출/이동이 안 보임 | 상시 `#lq-fx-mount` fixed + 높은 z-index가 WebGL 가림 | 노드 제거·실패 시에만 body에 임시 마운트 |
| 새로고침 시 매칭 없이 로비 | 세션 존재 시 `resumeFromLobby` | 항상 매칭 연출 후 진행 |
| 도전 후 멈춤·빈 화면 | `levelConfig` 미검증 `.find` 예외 / 토스트·캔버스가 포인터 가로챔 | 배열 가드, pointer-events, 화면 전환 시 토스트 숨김 |
| 도전 시작은 되는데 다음 단계 무반응 | ATTEMPT flex/`overflow:hidden`으로 버튼 절단 | `.screen {min-height:0}`, `#screen-attempt {overflow-y:auto}`, 버튼 margin 재조정 |
| 토스트가 전혀 안 뜸 | `#toast` DOM 누락 | `index.html`에 복구(verification_checklist) |
| 매칭 그리드 아래 빈 박스·카드 간격 들쭉날쭉 | `#avatar-stack` flex:1·카드 margin 제각각 | flex:0 1 auto, `--section-gap`+`screen-body` gap, 푸터 분리 |
| 매칭 패널 오른쪽 넘침 | width:100%+좌우 margin | width:auto+stretch, 본문 padding으로 14px 통일 |
| 화면마다 하단 버튼 높이가 다름 | CTA가 본문 flex 흐름에만 존재 | `screen-body`+`screen-footer`, 토큰 패딩·`.btn-footer` 전폭 |
| 원/점선 격자가 오른쪽으로 치우쳐 보임 | box-shadow x 오프셋 | 수직 그림자만 |
| 계속 버튼 숨김에도 하단 빈 줄 | 푸터가 항상 패딩 유지 | `:has(#btn-matching-continue.hidden)` |
| 깨진 세이브·비현실 스냅샷 | localStorage만 믿고 진행 | load 직후 보정: 7클리어+거의 전원 생존 등이면 초기화; `clearsForRank[0]≠clearsCompleted`면 동기 |
| 보상 확인 후 상태 꼬임 | 스토리지만 삭제 | `resetToFreshEvent` + MATCHING + `startMatchingAnimated` |
| 새로고침 ≠ 초기화 | 의도적 영속성 | 완전 리셋은 보상 확인 또는 `#btn-session-reset` |
| 연출/HUD 티어 불일치 | 씬 기본 playerClears=0 | `mountLavaScene` 옵션으로 `clearsCompleted` 또는 `clearsCompleted-1` 전달 |
| 생존자마다 다른 돌칸 | 옛 로직이 인덱스 기반 티어 | 전원 `playerClears` 동일 티어로 `computeClusterTargets` |
| 빌드 파싱 에러 | 잘못된 JSDoc/쉼표 삽입 등 | `npm run build`로 문법 검증 습관화 |

**아직 본편에서 하지 않은 일(의도적 공백)**

- 실매칭 / 백엔드 세션 / WebSocket
- Dust 파티클 등 문서 초안에만 있는 과장된 이펙트
- `lq_player_v1` 별도 사용처(함수만 존재)
